"""
================================================================================
butterfly_proxy_europe.py — Asifah Analytics
================================================================================
BUTTERFLY PROXY (Europe Backend)

Mirrors jawboning_proxy_wha.py architecture. The Europe backend's trackers (US,
Cuba, Mexico, Venezuela, Peru, Chile, etc.) call this proxy to fetch their
butterfly signal bundle. Proxy:
  1. Checks Europe local Redis for cached bundle
  2. If fresh (< 1h), returns it immediately
  3. If stale or missing, HTTP-fetches from ME backend's /api/butterfly/read
  4. Caches the fresh bundle to Europe local Redis
...
--------------------
    from butterfly_proxy_europe import read_butterfly_signals_via_proxy

    bundle = read_butterfly_signals_via_proxy(consumer_theater='russia')
...
---------------------
    GET /api/europe/butterfly/<consumer_theater>     — proxy passthrough
    GET /api/europe/butterfly-debug                  — diagnostic
...
# ============================================================================
# REDIS HELPERS (local Europe Redis — for caching the proxy response)
# ============================================================================

def _cache_key(consumer_theater):
    return f"butterfly:europe:{consumer_theater}:cached"


def _redis_load(consumer_theater):
    """Load cached butterfly bundle from Europe Redis. Returns None if missing or stale."""
    ...
    try:
        ...
        print(f"[Butterfly Proxy Europe] Redis load error: {e}")
        return None


def _redis_save(consumer_theater, bundle):
    """Save butterfly bundle to Europe Redis with TTL."""
    if not (UPSTASH_REDIS_URL and UPSTASH_REDIS_TOKEN):
        return
    try:
        ...
        print(f"[Butterfly Proxy Europe] Redis save error: {e}")
...
def _fetch_from_me(consumer_theater):
    ...
    try:
        ...
        if not resp.ok:
            print(f"[Butterfly Proxy Europe] ME backend HTTP {resp.status_code} "
                  f"for {consumer_theater}")
            return None
        return resp.json()
    except requests.Timeout:
        print(f"[Butterfly Proxy Europe] Timeout calling ME backend "
              f"for {consumer_theater} (>{FETCH_TIMEOUT_SECONDS}s)")
        return None
    except Exception as e:
        print(f"[Butterfly Proxy Europe] ME backend error for {consumer_theater}: "
...
def read_butterfly_signals_via_proxy(consumer_theater, force=False):
    ...
    Args:
        consumer_theater: str — 'russia', 'belarus', 'ukraine', 'greenland', etc.
    ...
    try:
        ...
        try:
            ...
            print(f"[Butterfly Proxy Europe] ⏱️ Hard timeout fetching {consumer_theater}")
            fresh = None
    except Exception as e:
        print(f"[Butterfly Proxy Europe] Fetch executor error: {e}")
        fresh = None
    finally:
...
def register_butterfly_proxy(app):
    """Register butterfly proxy endpoints on the Europe backend Flask app."""
    from flask import jsonify, request

    @app.route('/api/europe/butterfly/<consumer_theater>', methods=['GET', 'OPTIONS'])
    def api_europe_butterfly(consumer_theater):
    ...
    @app.route('/api/europe/butterfly-debug', methods=['GET'])
    def api_europe_butterfly_debug():
        return jsonify({
            'success':           True,
            ...
            'service':           'butterfly_proxy_europe',
            'version':           '1.0.0',
        })

    print("[Butterfly Proxy Europe] ✅ Endpoints registered:")
    print("  GET /api/europe/butterfly/<consumer_theater>")
    print("  GET /api/europe/butterfly-debug")
